...
--
-- Host: localhost    Database: raveapp
-- ------------------------------------------------------
...
--
-- Dumping routines for database 'raveapp'
--
...
BEGIN
	...
	#declaro handler para cuando termina el fetch
...
BEGIN
	...
	#borro generos y artistas para reinsertar
...
begin
    ...
    #set randomString = LEFT(UUID(), 8);
   #FLOOR(piso+RAND()*(techo-piso+1))
   	
       /* INSERT INTO raveapp.tblartistafav (idartista_fav, idusuario, idartista)
        SELECT UUID(), 
               (SELECT u.idusuario FROM tblusuario u ORDER BY RAND() LIMIT 1), 
               (SELECT a.idartista FROM tblartista a ORDER BY RAND() LIMIT 1);
        */
...
/*DECLARE done TINYINT DEFAULT FALSE;
declare v_idusuario varchar(36) default '';
...
v_loop: 
LOOP
  FETCH NEXT FROM v_cursor INTO v_idusuario;
  IF done THEN 
    LEAVE v_loop; 
  ELSE
    insert into tblrol_usuario (idrol_usuario, idusuario, cdrol)
values( uuid(), v_idusuario, 0);
